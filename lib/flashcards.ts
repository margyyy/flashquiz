...
export const flashcardSets = [
  ...
  },
  {
    ...
    cards: [
      {
        id: 238,
        front: "Quali sono i due protocolli principali del Livello di Trasporto e qual è la loro differenza fondamentale?",
        back: "I protocolli principali sono il TCP e l'UDP. Il TCP è un protocollo affidabile e orientato alla connessione (connection-oriented) che garantisce la consegna dei segmenti. L'UDP è connectionless e non affidabile, poiché i segmenti persi non vengono recuperati.",
      },
      {
        id: 239,
        front: "A cosa servono le porte nel Livello di Trasporto e come sono suddivise tra statiche e dinamiche?",
        back: "Le porte (rappresentate su 16 bit, per un totale di 65536 disponibili) servono come identificatori dei processi coinvolti nella comunicazione. Si dividono in porte statiche (o note), che vanno da 0 a 1023 e sono assegnate da uno standard ai server, e porte dinamiche, che vanno da 1024 a 65535 e sono assegnate dal sistema operativo al client.",
      },
      {
        id: 240,
        front: "Qual è il ruolo della porta 80 nell'ambito delle porte statiche?",
        back: "La porta 80 è un classico esempio di porta statica (nota). Viene utilizzata come porta di destinazione standard per i server web che comunicano tramite protocollo HTTP.",
      },
      {
        id: 241,
        front: "Quanto è grande l'header del protocollo TCP e quali sono alcuni dei suoi campi principali?",
        back: "L'header del protocollo TCP ha una dimensione fissa di 20 byte. Tra i suoi campi principali ci sono la Porta Destinazione, la Porta Sorgente, il Sequence Number, l'Acknowledge Number, e il Checksum.",
      },
      {
        id: 242,
        front: "Cos'è il Maximum Segment Size (MSS) e in che modo è correlato al Sequence Number?",
        back: "L'MSS (Maximum Segment Size) è la dimensione massima del segmento trasmesso e viene deciso in modo che sia compatibile con l'MTU della rete. Il Sequence Number indica la posizione del primo byte del segmento nel messaggio originale; per ogni nuovo segmento, questo numero viene incrementato di una quantità pari all'MSS.",
      },
      {
        id: 243,
        front: "A cosa serve il campo Acknowledge Number (ACK) all'interno dell'header TCP?",
        back: "L'Acknowledge Number contiene il Sequence Number del prossimo byte che la destinazione si aspetta di ricevere. Funge da riscontro (ACK) per garantire la corretta consegna e fa capire alla sorgente da quale pacchetto deve continuare a trasmettere.",
      },
      {
        id: 244,
        front: "Come agisce il meccanismo del Checksum all'interno dei segmenti TCP?",
        back: "Il Checksum controlla la presenza di errori nel segmento applicando una funzione matematica che restituisce un output di dimensione fissa. Se alla ricezione il confronto fallisce indicando la sicura presenza di errori, il segmento viene scartato.",
      },
      {
        id: 245,
        front: "Quali sono i tre passaggi del Three-Way Handshake per instaurare una connessione TCP?",
        back: "1) Il Client invia un segmento con flag SYN e il proprio Initial Sequence Number (ISNc). 2) Il Server risponde con SYN-ACK, indicando il suo ISNs e confermando l'ISNc. 3) Il Client manda un ACK per riscontrare l'ISNs del Server.",
      },
      {
        id: 246,
        front: "Come avviene la chiusura di una connessione TCP al termine dello scambio di messaggi?",
        back: "Essendo il canale TCP bidirezionale, la chiusura deve avvenire in entrambe le direzioni. La procedura richiede l'invio di un segmento con il flag FIN (per richiedere la chiusura) a cui l'altra parte risponde con un ACK di conferma, per entrambi i lati della comunicazione.",
      },
      {
        id: 247,
        front: "Cos'è il Retransmission Time Out (RTO) e come viene calcolato utilizzando lo SRTT?",
        back: "L'RTO è il tempo massimo di attesa per ricevere un riscontro prima di effettuare una ritrasmissione del pacchetto. Poiché il Round Trip Time (RTT) è variabile, l'RTO si calcola partendo dallo SRTT (Smoothed RTT), che è una media mobile esponenziale ponderata (EWMA) dei valori RTT istantanei e precedenti. Nota: se il primo SYN non va a buon fine, viene utilizzato un tempo di default pari a 500ms.",
      },
      {
        id: 248,
        front: "In che modo la Finestra Scorrevole permette di aumentare la velocità di trasmissione?",
        back: "Invece di trasmettere un singolo segmento e aspettare il riscontro impiegando l'intero RTT, il meccanismo a finestra scorrevole permette di trasmettere più segmenti contemporaneamente. La finestra avanza (scorre) facendola scorrere ogni volta che arriva il corretto segmento di ACK.",
      },
      {
        id: 249,
        front: "Come evolve la dimensione della Congestion Window (CWND) durante la fase di Slow Start e di Congestion Avoidance?",
        back: "Nello 'Slow Start', la CWND raddoppia a ogni RTT, aumentando di 1 segmento per ogni ACK ricevuto. Quando la CWND raggiunge la soglia di ssthresh, il protocollo passa alla fase di 'Congestion Avoidance' dove la crescita diventa più lenta, aumentando solo di una frazione pari a 1/CWND per ogni riscontro.",
      },
      {
        id: 250,
        front: "Che differenza c'è nella gestione delle perdite tra TCP Vanilla e la variante Fast Retransmit/Fast Recovery?",
        back: "Nel TCP Vanilla, allo scadere del timer RTO, la perdita si considera grave: la CWND viene ridotta a 1 e si riparte in Slow Start. Nel Fast Retransmit/Fast Recovery, se arrivano 3 ACK duplicati, la finestra viene invece solo dimezzata (CWND = CWND/2) evitando di ripartire da 1.",
      },
      {
        id: 251,
        front: "Quali sono le tre variabili fondamentali gestite dall'Algoritmo di Congestione del TCP?",
        back: "L'algoritmo di congestione si basa su: CWND (la dimensione attuale della finestra), RCVWND (la finestra di ricezione comunicata dal ricevente) e SSTHRESH (la soglia usata per distinguere quando usare Slow Start o Congestion Avoidance).",
      },
      {
        id: 252,
        front: "Com'è strutturato l'header del protocollo UDP?",
        back: "L'header dell'UDP è molto essenziale e contiene solamente quattro campi: Porta Sorgente, Porta Destinazione, Lunghezza (Length) dei dati e Checksum.",
      },
      {
        id: 253,
        front: "Per quali tipi di applicazioni è solitamente impiegato il protocollo UDP?",
        back: "L'UDP è utilizzato principalmente per applicazioni che riescono a tollerare la perdita di segmenti ma necessitano di tempi rapidi di trasmissione (non ritardati da ritrasmissioni o da messaggi preliminari per il setup della connessione), come ad esempio le comunicazioni e gli stream Audio o Video.",
      },
    ],
  },
  {
    id: 221,
    name: "5 - Livello Di Rete",
    cards: [
      {
        id: 222,
